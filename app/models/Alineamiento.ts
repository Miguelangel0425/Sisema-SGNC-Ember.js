...
export class Alineamiento {
  ...
  /** El documento permite máximo 2 filas ODS (ODS / ODS_2). */
  public agregarODS(o: ODS): void {
    if (this._ods.length >= 2) {
      throw new Error(
        'Solo se permiten hasta 2 objetivos ODS por nota conceptual.',
      );
    }
    this._ods.push(o);
  }

  public removerODS(codigo: string): void {
    this._ods = this._ods.filter((o) => o.codigo !== codigo);
  }

  /** Máximo 2 líneas de investigación, tal como el documento define 2 comboBox. */
  public agregarLineaInvestigacion(linea: string): void {
    if (this._lineasInvestigacion.length >= 2) {
      throw new Error('Solo se permiten hasta 2 líneas de investigación.');
    }
  ...
  public removerLineaInvestigacion(linea: string): void {
    this._lineasInvestigacion = this._lineasInvestigacion.filter(
      (l) => l !== linea,
    );
  }

  public validarAlMenosUnAmbito(): boolean {
    return this._ambitosPrioritarios.some((a) => a.aplica);
  }
}
