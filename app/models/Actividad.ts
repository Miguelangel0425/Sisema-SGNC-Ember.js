import type { IActividad } from '../types/IActividad';
...
export class Actividad implements IActividad {
  ...
  public duracionDias(): number {
    const ms = this._fechaFin.getTime() - this._fechaInicio.getTime();
    return Math.round(ms / (1000 * 60 * 60 * 24));
  }

  /** "La fecha fin de cada actividad debe ser posterior a su inicio." */
  public esValida(): boolean {
    return (
      this._nombre.trim().length > 0 &&
      this._fechaFin.getTime() > this._fechaInicio.getTime()
    );
  }
}
