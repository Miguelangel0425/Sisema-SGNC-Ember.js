...
export class Validator {
  ...
  public static validarTelefonoValido(telefono: string): IResultadoValidacion {
    if (!Validator.REGEX_TELEFONO.test(telefono.trim())) {
      return {
        valido: false,
        mensaje: 'El teléfono celular no tiene un formato válido.',
      };
    }
    return { valido: true };
  }

  public static validarCampoObligatorio(
    valor: string,
    ...
    if (!valor || valor.trim().length === 0) {
      return {
        valido: false,
        mensaje: `El campo "${etiquetaCampo}" es obligatorio.`,
      };
    }
    return { valido: true };
  }

  public static validarCodigoUnico(
    codigo: string,
    ...
    if (codigosExistentes.includes(codigo)) {
      return {
        valido: false,
        mensaje:
          'El código ingresado ya existe. No se permiten códigos repetidos.',
      };
    }
    return { valido: true };
  }

  public static validarNombreConvocatoriaUnico(
    nombre: string,
    existe: boolean,
  ): IResultadoValidacion {
    if (existe) {
      return {
        valido: false,
        mensaje: 'Ya existe una convocatoria registrada con ese nombre.',
      };
    }
    return { valido: true };
  }

  public static validarAlMenosUnaActividad(
    cantidad: number,
  ): IResultadoValidacion {
    if (cantidad === 0) {
      return {
