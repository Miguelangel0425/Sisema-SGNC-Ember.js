...
import { LIMITE_PRESUPUESTO_USD } from '../models/Presupuesto';
...
export class PresupuestoValidator {
  ...
  public static validarValorUnitario(
    valorUnitario: number,
  ): IResultadoValidacion {
    if (valorUnitario < 0) {
      return {
        valido: false,
        mensaje: 'El valor unitario debe ser mayor o igual a cero.',
      };
    }
    return { valido: true };
  }
  ...
  public static validarAlMenosUnItem(
    cantidadItems: number,
  ): IResultadoValidacion {
    if (cantidadItems === 0) {
      return {
        valido: false,
        mensaje: 'Debe existir al menos un ítem presupuestario.',
      };
    }
    return { valido: true };
  }
